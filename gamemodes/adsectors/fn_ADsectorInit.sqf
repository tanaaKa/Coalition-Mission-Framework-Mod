...
tnk_sectorSetup = 
{
  //Check if sectors exists when set to not random
  if (!_randomSectors && isNil "sector1") exitWith {
    "[CMF Attack & Defend] ERROR: Pre-placed sectors not detected. Please place at least one trigger called sector1" remoteExec ["systemChat"];
  };

  //Make sure both sides aren't set to the same objective
  if (attackerSide == defenderSide || defenderSide == attackerSide) exitWith {
    "[CMF Attack & Defend] ERROR: The same side cannot be both attacker and defender" remoteExec ["systemChat"];
  };

  //Main setup
  if (_randomSectors) then {
    "[CMF Attack & Defend] Random sectors selected. Setting up game mode" remoteExec ["systemChat"];
    call tnk_createRandomSectors;
  } else {
    "[CMF Attack & Defend] Pre-placed sectors selected. Setting up game mode" remoteExec ["systemChat"];
    call tnk_createSectors;
  };
};

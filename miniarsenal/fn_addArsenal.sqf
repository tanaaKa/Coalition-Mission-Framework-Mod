...
if (CBA_missionTime > 10) exitWith {systemChat "[COA MA] MiniArsenal is disabled due to JIPing"};
...
tnk_closeMA =
{
	...
	//JIP support check
	if (startingMags == 0 || startingMags == nil) exitWith {systemChat "[COA MA] Miniarsenal detected that you had zero mags originally. Reseting gear just in case."; [player] call potato_assignGear_fnc_assignGearMan;};
	
	//Check for extra magazines and reset gear if so
	endingMags = count magazines player;
	
	if (endingMags > startingMags) then {

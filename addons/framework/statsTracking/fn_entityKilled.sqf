...
switch true do
{
	...
	case (isPlayer _instigator && !(_sideKilled in ["potato_w", "potato_i", "potato_e"])):{
		[1, 8, (getPlayerUID _instigator)] call CMF_fnc_updateStatArray;
	};
	...
	case ((isPlayer _killed && isPlayer _instigator) && _sideKilled != _sideInstigator):{
		[1, 4, (getPlayerUID _killed)] call CMF_fnc_updateStatArray;
		[1, 3, (getPlayerUID _instigator)] call CMF_fnc_updateStatArray;
	};
	//If both parties are players and are on the same side, then it's a FF kill
	case ((isPlayer _killed && isPlayer _instigator) && _sideKilled == _sideInstigator):{
		[1, 7, (getPlayerUID _instigator)] call CMF_fnc_updateStatArray;
	};
	...
	case (isPlayer _killed &&  !isPlayer _instigator && _sideKilled != _sideInstigator):{
		[1, 6, (getPlayerUID _killed)] call CMF_fnc_updateStatArray;
	};
	...
	case (!isPlayer _killed &&  isPlayer _instigator && _sideKilled != _sideInstigator):{
		[1, 5, (getPlayerUID _instigator)] call CMF_fnc_updateStatArray;
	};
};
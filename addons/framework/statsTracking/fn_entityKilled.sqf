...
switch true do
{
	...
	case (isPlayer _instigator && !(_sideKilled in ["potato_w", "potato_i", "potato_e"])):{
		[1, 8, (_instigator getVariable "Player_UID")] call CMF_fnc_updateStatArray;
	};
	...
	case ((isPlayer _killed && isPlayer _instigator) && _sideKilled != _sideInstigator):{
		[1, 4, (_killed getVariable "Player_UID")] call CMF_fnc_updateStatArray;
		[1, 3, (_instigator getVariable "Player_UID")] call CMF_fnc_updateStatArray;
	};
	//If both parties are players and are on the same side, then it's a FF kill
	case ((isPlayer _killed && isPlayer _instigator) && _sideKilled == _sideInstigator):{
		[1, 7, (_instigator getVariable "Player_UID")] call CMF_fnc_updateStatArray;
	};
	...
	case (isPlayer _killed &&  !isPlayer _instigator && _sideKilled != _sideInstigator):{
		[1, 6, (_killed getVariable "Player_UID")] call CMF_fnc_updateStatArray;
	};
	...
	case (!isPlayer _killed &&  isPlayer _instigator && _sideKilled != _sideInstigator):{
		[1, 5, (_instigator getVariable "Player_UID")] call CMF_fnc_updateStatArray;
	};
};
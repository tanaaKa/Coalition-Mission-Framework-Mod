...
private _objs = nearestObjects [getPos opfvehpad, [], 8];
...
private _transport = (opftransport select [2]); //gets rid of prefix 'I_'
private _transport = (toLower _transport);
private _tank = (opfarmor select [2]);
private _tank = (toLower _tank);
private _boat = (boat select [2]);
private _boat = (toLower _boat);
private _ftvic = (opfftveh select [2]);

...
if (count _pos > 1) then
{
	...
	if (isNil "_inv") then
	{
		...
		if (isClass(configFile >> "CfgPatches" >> "kat_main")) then
		{
			...
			_fh addItemCargoGlobal ["kat_IV_16", 100];
			...
			_fh addItemCargoGlobal ["kat_naloxone", 35];
			_fh addItemCargoGlobal ["kat_nitroglycerin", 35];
			_fh addItemCargoGlobal ["kat_norepinephrine", 35];
			_fh addItemCargoGlobal ["kat_phenylephrine", 35];
			_fh addItemCargoGlobal ["kat_amiodarone", 35];
			_fh addItemCargoGlobal ["kat_atropine", 35];
			_fh addItemCargoGlobal ["kat_TXA", 30];
			_fh addItemCargoGlobal ["kat_Carbonate", 10];
			_fh addItemCargoGlobal ["kat_Painkiller", 10];
			_fh addItemCargoGlobal ["kat_retractor", 20];
			_fh addItemCargoGlobal ["kat_scalpel", 20;
			_fh addItemCargoGlobal ["kat_nalbuphine", 25];
			_fh addItemCargoGlobal ["kat_lorazepam", 25];
			_fh addItemCargoGlobal ["kat_flumazenil", 35];
			_fh addItemCargoGlobal ["kat_kat_fentanyl", 25];
			_fh addItemCargoGlobal ["kat_etomidate", 25];
			_fh addItemCargoGlobal ["kat_EACA", 30];
			_fh addItemCargoGlobal ["kat_vacuum", 10];
			_fh addItemCargoGlobal ["kat_clamp", 20];
			_fh addItemCargoGlobal ["kat_plate", 40];
			_fh addItemCargoGlobal ["Attachable_Helistretcher", 4];
		};
